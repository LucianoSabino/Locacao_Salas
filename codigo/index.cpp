...
//	Iniciando as matrizes e variveis
...
//	Funcao que verifica se a quatidade de turma, horario e dia ja esta peenchido 
void procuraSalas(){
	...
	if(contador70 >= 2 && capacidadeAlunos == 70){
		...
		std::cout << "\nDeseja alocar a turma no horario " << horarioNovo << "(S/N)? ";
		char opcao;
		std::cin >> opcao;
...
int main() {
	...
	std::cout << "-------------------- A tabela esta organizada com forme o horario e todos os dias da semana corespondente --------------------";
	for (int h = 0; h < 4; h++) {
		std::cout << "\nHorario: " << hora[h] << "\n";
